...
<?php
	...
	function wppizza_report_export($dataset){		
		...
		/**************************************************************************
			sales by date
		**************************************************************************/
		$result='"Range: '.$startdate.' - '.$enddate.'"'.PHP_EOL.PHP_EOL;
		$result.='"sales by dates"'.PHP_EOL;
		/*sales*/
		$result.='"date", "sales value(incl. taxes, charges and discounts)", "items order value", "number of sales", "number of items sold"  , "tax on order"  '.PHP_EOL;
		$d=0;
		/**sum it up*/
		$sales_value_total=0;
		$items_value_total=0;
		...
		/**sums of it all*/
		$result.='"", "total", "total", "total", "total", "total" '.PHP_EOL;
		...
		**************************************************************************/
		if(is_array($dataset['items_summary']) && count($dataset['items_summary'])>0){
			...
			$result.='"sales by item"'.PHP_EOL;
			/*items*/
			$result.='"quantity", "item", "total value"'.PHP_EOL;
			$totalNumberItems=0;
			$totalSalesItems=0;
			foreach($dataset['items_summary'] as $uniqueItem=>$itemDetails){
				$result.=$itemDetails['quantity']  . $delimiter . $itemDetails['name'] . $delimiter . $itemDetails['pricetotal'];
				$result.=PHP_EOL;
				/*add it up*/
		...
		**************************************************************************/
		if(is_array($dataset['gateways_summary']) && count($dataset['gateways_summary'])>0 && !defined('WPPIZZA_OMIT_REPORT_GATEWAYS_SUMMARY')){
			$result.=PHP_EOL.'"payment type"'.PHP_EOL;	
			/*items*/
			$result.='"type", "total value"'.PHP_EOL;			
		...
		**************************************************************************/
		if(is_array($dataset['order_status_summary']) && count($dataset['order_status_summary'])>0 && !defined('WPPIZZA_OMIT_REPORT_ORDER_STATUS_SUMMARY')){
			$result.=PHP_EOL.'"order status"'.PHP_EOL;	
			/*items*/
			$result.='"status", "count", "total value"'.PHP_EOL;			
